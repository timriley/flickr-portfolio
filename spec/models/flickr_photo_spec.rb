...
describe FlickrPhoto do
  describe "created with a flickr id" do
    before(:each) do
      ...
      photos_container.stub!(:getSizes).with(:photo_id => '123').and_return([@flickr_url_thumb, @flickr_url_med, @flickr_url_orig])
  ...
  describe "when finding flickr photos by user and tag" do
    it "should build new flickr photo objects for each photo returned from flickr"
    
    it "should return an empty array if no photos were found"
